...
import { emitAuditEvent } from "../security/audit";
...
/**
 * Framework-agnostic webhook processor for Next.js Route Handlers.
 * Caller has already parsed body + headers via `await req.json()` etc.
 */
export async function processWebhookRaw(opts: {
  ...
  try {
    const result = await processWebhook(
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      opts.body as any,
      { ...(opts.tenantId ? { tenantId: opts.tenantId } : {}), ...opts.query }
    );

    if (!result.plugin) return { handled: false };

    const eventId =
      (opts.headers["x-goog-message-number"] as string | undefined) ??
      (opts.headers["x-webhook-id"] as string | undefined) ??
      crypto.randomUUID();

    const resolvedTenant = opts.tenantId ?? (result as unknown as Record<string, string>).tenantId ?? "unknown";
    const isDuplicate = webhookStore.isDuplicate(resolvedTenant, result.plugin, eventId);
    ...
    if (isDuplicate) return { handled: true, plugin: result.plugin, action: result.action ?? undefined, duplicate: true };
