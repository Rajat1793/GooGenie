...
import { useCalendarEvents, useDeleteCalendarEvent } from "../api/hooks";
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: "var(--c-surface-container-low)", border: "1px solid var(--c-outline-variant)" }}>
      ...
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid var(--c-outline-variant)" }}>
        <div className="flex items-center gap-3">
          <button onClick={() => shiftMonth(-1)} className="btn-ghost p-1.5" title="Previous month"><Icon name="chevron_left" /></button>
          <h3 className="font-headline text-lg" style={{ color: "var(--c-on-surface)" }}>{monthLabel}</h3>
          <button onClick={() => shiftMonth(1)} className="btn-ghost p-1.5" title="Next month"><Icon name="chevron_right" /></button>
        </div>
              ...
              <div className="space-y-0.5">
                {dayEvents.slice(0, 3).map((e) => (
                  <button
                    ...
                    title={e.title}
                  >
                    {e.meetLink && <Icon name="videocam" className="text-[10px]" />}
    ...
    <div className="flex gap-4 rounded-2xl px-5 py-4 transition-all duration-150 nimbus-card nimbus-card-hover" style={{ borderLeft: isToday ? "4px solid var(--c-primary)" : "4px solid transparent" }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1 flex-wrap">
          <span className="flex items-center gap-1 text-xs" style={{ color: "var(--c-on-surface-variant)" }}><Icon name="schedule" className="text-sm" />{timeStr}</span>
          <span className="flex items-center gap-1 text-xs" style={{ color: "var(--c-on-surface-variant)" }}><Icon name="hourglass" className="text-sm" />{durationMin}m</span>
        </div>
...
export function CalendarPage() {
  ...
  useEffect(() => {
    ...
    const saved = window.localStorage.getItem(STORAGE_KEYS.calendarView) as "list" | "month" | null;
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(STORAGE_KEYS.calendarView, viewMode);
  }, [viewMode]);
    ...
    <div>
      ...
      <div className="ai-insight mb-6">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: "color-mix(in srgb, var(--c-primary) 15%, transparent)" }}>
          <Icon name="auto_awesome" style={{ color: "var(--c-primary)", fontVariationSettings: "FILL 1" }} />
        </div>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="flex rounded-full overflow-hidden" style={{ border: "1px solid var(--c-outline-variant)" }}>
            {(["list", "month"] as const).map((m) => (
              ...
              >
                <Icon name={m === "list" ? "view_list" : "calendar_view_month"} className="text-[16px]" />
                {m === "list" ? "List" : "Month"}
              </button>
            ))}
          </div>
        ...
        <div className="relative mt-3 mb-4">
          ...
          />
          {(search || serverSearch) && <button onClick={() => { setSearch(""); setServerSearch(""); }} className="absolute right-2 top-1/2 -translate-y-1/2" style={{ color: "var(--c-outline)" }}><Icon name="close" className="text-base" /></button>}
        </div>
      </div>
        ...
        <>
          ...
          {!loading && filtered.length === 0 && !error && (
            <div className="flex flex-col items-center justify-center py-24 gap-4" style={{ color: "var(--c-on-surface-variant)" }}>
              <Icon name="calendar_today" className="text-6xl" style={{ opacity: 0.3 }} />
              <p className="font-headline text-2xl">No events {filter === "today" ? "today" : filter === "upcoming" ? "upcoming" : "yet"}</p>
              <button onClick={() => setCreating(true)} className="btn-primary mt-2"><Icon name="add" className="text-sm" />Create an event</button>
            </div>
          )}
